...
def index():
	try:
		...
		if(campusid is None and bc is None):
			return jsonify({"status":False,"error":"campus and building should not be null"}), 400
		if(campusid is None):
			return jsonify({"status":False,"error":"campus should not be null"}), 400
		if(bc is None):
			return jsonify({"status":False,"error":"building should not be null"}), 400
...
def schedule():
	try:
		...
		if(campusid is None and roomid is None):
			return jsonify({"status":False,"error":"campus and room should not be null"}), 400
		if(campusid is None):
			return jsonify({"status":False,"error":"campus should not be null"}), 400
		if(roomid is None):
