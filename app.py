...
if not os.path.exists(DATABASE):
	...
	cur = conn.cursor()
	cur.execute("CREATE TABLE users (username TEXT, password TEXT, permission INTEGER, token TEXT);")
	cur.execute("CREATE TABLE log (nodeId TEXT, volt FLOAT, amp FLOAT, watt FLOAT, ts INTEGER);")
	cur.execute("CREATE TABLE report (nodeId TEXT, name TEXT, message TEXT, ts INTEGER);")
	cur.execute("CREATE TABLE config (nodeId TEXT, campus INTEGER, building TEXT, room TEXT, name TEXT);")
	conn.commit()
